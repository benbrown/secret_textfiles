...
const processEmbeds = async(content) => {
  ...
  if (urls = content.match(/^(http.*?instagram.*?)$/img)) {
    for (let url of urls) {
      try {
        let embed = await fetchInstaEmbed(url);
        if (embed !== false) {
          content = content.replace(url, embed);
        }
      } catch(err) {
